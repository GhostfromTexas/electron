...
URLRequestStreamJob::URLRequestStreamJob(
      ...
      net::NetworkDelegate *network_delegate) : JsAsker<net::URLRequestJob>(
        request,
        network_delegate),
      buffer_(),
      ended_(false),
...
void URLRequestStreamJob::BeforeStartInUI(
  ...
  status.append(" ");
  status.append(net::GetHttpReasonPhrase(
  ...
  if (status_code == 204 || !opts.Get("body", &value) || !value->IsObject()) {
    ended_ = true;
    return;
  }

  mate::Dictionary body(isolate,
      value->ToObject(isolate->GetCurrentContext()).ToLocalChecked());
  if (!body.Get("on", &value) || !value->IsFunction() ||
      !body.Get("once", &value) || !value->IsFunction() ||
      !body.Get("removeAllListeners", &value) || !value->IsFunction()) {
    // invalid body
    ended_ = true;
    errored_ = true;
    return;
  ...
  subscriber_.reset(new mate::EventSubscriber<URLRequestStreamJob>(this,
        isolate, body.GetHandle()));
...
int URLRequestStreamJob::ReadRawData(net::IOBuffer* dest, int dest_size) {
  content::BrowserThread::PostTask(
      ...
      base::Bind(&URLRequestStreamJob::CopyMoreData,
